...
- Shrinkage: 추가로 추정되어지는 모델의 영향력을 줄이는 방법. Learning rate 파라미터로 강도 조절. 모델을 어느 정도로 업데이트할것인지 결정하여 과적합 방지. 
...
### 4.1. 데이터셋 분할
0. validation: 학습중, test: 학습을 마친후
- 검증, 평가데이터를 잘 설정하는 것이 중요!
1. Holdout
- 주로 8:2 랜덤 분할, 특히 시계열데이터에 많이 활용
- 장점: 속도 빠르고 간편
- 단점: 성능이 일반적으로 낮은편. 일부데이터(검증데이터)가 학습에 참여하지 못했으므로

2. K-Fold
- K번의 Holdout으로 데이터를 분리. K-1개를 학습세트로, 1개를 검증세트로 사용
- 장점: 모든 데이터를 훈련에 활용, 신뢰성 보장, 높은 성능
- 단점: 소요시간 증가, K의 선택 필요
- 대용량 데이터 실험에서는 Holdout, 배포/제출에서는 K-Fold 

3. Stratified K-Fold: 많이 활용
- K-Fold와 동일하지만, Fold별 Y의 비율도 동일하게 분할. 데이터 양이 적거나 불균형이 심한 데이터에 대해 분포가 크게 달라지는 현상을 방지. 불균형에서 오는 부분 일부 해소.

4. Group K-Fold: 특수상황, 주로 의료에서 활용
- 일반적으로 새로운 사람 예측, 환자 데이터에 사용 (같은 환자의 정보가 train, valid에 모두 들어있으면 새로운 환자의 질병을 예측해야하는 데이터에서 문제가 발생할 수 있음)
- train, valid에 같은 값이 들어가지 않게 Group 구성 후, Fold별로 Group 분배

5. Time Series Split
- 과거 데이터를 통해 미래 예측하는 구조를 유지한 채 Fold를 나누는 방법

## 5. 대회에서 사용하는 실전 팁

### 5.1. 실험 전 고려사항
- 디버깅: 프로그래밍에서 오류를 찾아내고 수정하는 과정(샘플 데이터, 작은 모델, 하이퍼파라미터 축소)
- 시드 고정의 필요성: 공정한 비교를 위해 재현이 되어야함. 
- 실험기록: 노션, 구글 스프레드시트, 일반적으로 하나의 요소 실험 추천 (조건을 하나씩 변경해가면서 실험)

### 5.2. 앙상블

1. K-Fold 앙상블: 데이터 측면의 결합

2. 모델 앙상블: 예) LGBM + Catboost, Linear Regression + Boosting, Neural Network + Boosting
- 서로 달라야함: correlation 관점에서 확인
- 기본 베이스 성능이 어느정도 보장되어야 앙상블 이후 성능 개선 효과 기대
- Stacking: 모델 예측 결과를 피처로 활용. oof: out of fold

3. 랜덤성 측면: 
- Seed 앙상블: 시드만 동일하게 해서 앙상블

### 5.3. 추가데이터
- Pseudo Labeling: 데이터셋에 가짜 레이블 부여하는 방법. 
- 일반적 모델의 학습/예측 진행>예측한 평가 셋과 학습 셋을 결합>위 결과를 기반으로 새로운 모델 학습

### 5.4. 분석 도구
- EDA 자동화 도구: Dataprep.eda 데이터분석을 자동화해주는 도구
- Weights&Biases (WanDB): 기록/분석 시각화 도구 -> 실험기록 작성할 때 활용하면 좋음
