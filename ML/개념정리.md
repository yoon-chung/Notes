...
0.0. 매니폴드 가설: 일반적인 고차원 데이터들은 가능한 모든 데이터 공간에 완전히 고르게 분포하는게 아닌, 상대적으로 매우 낮은 차원으로 이해할 수 있는 매니폴드(곡면과 유사한 개념)을 이루고 있다. -> 이에 따라 비지도학습의 여러 연구분야가 가능
...
2. 기초방법론
  ...
  2-2. 연관규칙 (Association Rule Analysis)
  - DB내에서 동시에 등장하는 빈도가 높은 컨텐츠의 조합을 찾아내는 방법론
  
  2-3. 콘텐츠 기반 추천(CBF)
  
  2-4. 협업필터링: 사용자들 또는 아이템 사이 유사성 기반
  - 코사인 유사도: 두 벡터 사이 각도의 코사인 값 기준 (-1~1사이, 서로 유사할수록 1에 가까움)
  
    2-4-1. 기억 기반(Memory-based): 
    ...
    - 잠재요인 분석: 비지도학습의 임베딩과 같은 개념. SVD 행렬분해를 활용한 평점 예측
...
## 12. 시계열 데이터
- 규칙성분: 추세(trend), 계절성(seasonality), 순환성(cycle)
- 불규칙성분: 랜덤한 원인에 의한 변동. 예측불가한 노이즈

- 기술적 분석: 과거 데이터를 이해 목적. 이상치 탐지, 일반적 행동설명
- 예측적 분석: 미래 데이터 예측 목적

1. 특성
- 독립항등분포 IID: Independent and Identically Distributed가 성립하지 않는다! -> 일반적 ML 데이터와 달리, 과거가 미래에 영향을 주기 때문에 서로 다른 시점의 데이터가 독립이 아니다.
- 자기상관(Autocorrelation):  Corr($x_{t}$, $x_{t+k}$)
- 마르코프 속성: 미래에 대한 조건부 확률분포가 현재 상테에만 의존
- 정상성(Stationarity): 분포가, 시간에 따라 변치않고 동일하게 유지

2. 분석과정: 정상성 검정 -> 정상적데이터로 전처리 -> 모델 구성
3. 분석방법: 
- 성분분해: 계절성, 추세, 불규칙 성분으로 분해
- 이동평균(MA): 단기적 변동을 smoothing으로 제거, 장기적 추세 잘 확인가능
- 자기회귀모델(Auto-Regressive): 일정 스텝(p)만큼 과거 데이터들(t-p~ t-1)의 입력값으로 선형회귀모델을 학습하여 특정 스텝(t)를 예측

## 13. 모델 성능과 데이터셋 분할
1. 적절한 분할의 필요성: 손실함수가 0이라는 사실만으로, 모델이 잘 학습되었다고 판단할 수 없다.
- 데이터 누출(leakage): 원본데이터셋에 중복샘플/매우 유사한 샘플 포함
- 평가용 데이터셋의 대표성 부족: 너무 작거나 불균형해 신뢰도가 부족

- 평가용(test) vs 검증용(valid): valid dataset의 성능평가 결과는, 모델 개선에 사용됨.
예) 하이퍼파라미터 최적화로 모델 구조 결정, 학습 완료된 모델의 선별에 사용. 

- k-fold 교차검증(cross validation): 각 fold를 번갈아가며 valid dataset으로 사용해 총 k번의 학습과 검증을 진행한 뒤, 결과를 평균내어 모델의 최종 성능을 추정 
-> 효율적 학습, (데이터셋이 작을 경우) 편향 줄일 수 있음. 단, 데이터/모델 큰 경우 많은 리소스 필요

2. 모델의 일반화 성능
- 내삽(interpolation): train dataset 범위 내에서 새로운 데이터를 예측/분류
- 외삽(extrapolation): 범위를 넘어서는 새로운 데이터를 예측/분류
- 일반화 성능: 내삽+외삽 포함해 처음 접하는 데이터에서 얼마나 좋은 성능 보이는지
- test dataset: 외삽 성능까지 측정할 수 있게 최대한 다양한 샘플들 포함
- valid dataset: 내삽 성능 측정 위해 train dataset과 같은 분포 유지

3. 데이터셋 분할 방법: 데이터 크기가
- 적당(천~만개): 7:1:2
- 작다면(백 개 정도): 50:50 or 5-fold 교차검증
- 크다면(백만개 이상): 3분할 적당히. 어느정도 샘플 수가 확보된 이후에는, 양보다 품질 관리가 더 중요
- 시계열 데이터 검증: 
valid: 가장 최근 데이터, train:그보다 이전 데이터, test: 현 시점 이후 미래 데이터
- 계층적 분할: 
특정 클래스에 해당하는 샘플이 너무 적을 경우, 클래스별로 나눠 각각 랜덤분할 후 다시 합침

## 14. 정량평가 지표
1. 분류문제
- Confusion Matrix: 이진분류 문제에서 모델 예측치와 실제값에 따른 샘플의 수. FP, TN
- 정밀도(Precision): TP/ (TP+FP)
- 재현율(Recall): TP/ (TP+FN) => medical에서는 더 중요함
- F1 score(정밀도와 재현율의 조화평균)
-> 데이터 불균형 있는 경우, 정밀도/재현율/F1 score 모두 사용
- ROC Curve: 음성/양성 판단의 기준값의 변화에 따른 TP, FP 비율이 변하는 추세를 나타낸 곡선
- AUC (Area Under the ROC Curve): 정밀도+재현율 모두 중요하게 고려하는 정량평가 지표

2. 회귀문제
- MAE: 절대값, 오차가 0인지 아닌지 민감
- MSE: 제곱, 이상치 존재여부에 민감
- RMSE: 제곱근
- 편집거리(Edit Distance): 시계열에서 주로 사용, 몇번의 edit거쳐야 목표값으로 만들수 있는지

3. 컴퓨팅 자원 관련
- Latency(인퍼런스 시간): 샘플 한개 or 여러개 한번에 처리하는데 걸리는 시간
- Throughput(처리율): 단위시간당 처리할 수 있는 샘플 수
- Memory Usage: 점유하는 메모리 크기

4. 여러 지표를 함께 활용 가능: 
예) 수는 적지만 더 중요하다면, 해당 데이터셋에 가중평균하여 최종 모델선택에 사용
- 여러 정량평가 지표가 충돌 시: 최적화 지표 vs 충족 지표. 예) 정확도 vs GPU메모리

5. 목표 성능지표의 설정
- 베이스라인 스코어: 실험과정에서 비교용으로 사용
예) 랜덤베이스라인(아무거나 찍어서), 모델베이스라인: 나이브베이즈 분류기, k-최근접이웃 분류, 다중 로지스틱 회귀
- 리더보드: 연구자들이 각자 학습한 모델 성능을 공유하는 순위표.
예) SOTA(State of the Art: 리더보드에서 1위) 모델
- 베이즈 최적 에러: 현재 정량평가 방식에서 이론상 도달할 수 있는 이상적인 분류체계의 성능
- 휴먼 퍼포먼스: 예) 조류전문가들 팀이 논의해 결론 
- 지속적 정성평가로 정량평가를 보완/개선
- 트리거: 모델성능에 문제를 일으키는 인풋 데이터의 패턴, 트리거샘플만 모아 별도의 평가 dataset 구성

## 15. 모델 에러 분석
1. 과소적합: 큰 편향(Bias), 과적합: 큰 분산(Variance)
- 모델A: Train 99%, Valid 98%, Test 78% -> 최적화 잘됐지만 평가셋과 분포차이 존재
- 모델B: Tran 99%, Valid 83%, Test 73% -> 작은 편향, 큰 분산, 과적합
- 모델C: Train 70%, Valid 62%, Test 72% -> 큰 분산, 과소적합 (큰분산일때는 큰편향 별 의미x)
2. 과적합 문제 해결
- Regularization: 과적합 문제 해결방법, 모델 파라미터의 L1, L2 norm을 손실함수(MSE)값에 더해 학습과정에서 자연스럽게 파라미터 크기가 줄어서 모델 복잡도가 떨어지도록 유도
- Lasso (L1 norm, 절대값 합), Ridge (L2 norm, 제곱합)
- Augmentation(데이터 증강): 정답 라벨이 바뀌지 않는 한도에서, 인풋 데이터를 여러방법으로 변형시켜 새로운 데이터로 활용. (이미지: 좌우/상하반전, 밝기, 색상 조정, 정형 데이터: 랜덤 노이즈값 더하거나 값을 서로 교체) -> 도메인 지식 활용
- Learning Curve: Train set, Validation set에서 학습 진행상태에 따라 변화하는 손실함수 값 변화
- Early Stopping: Train set, Valid set의 손실함수 값차이가 커질때 학습을 일찍 종료
3. 편향-분산 trade-off
- 하지만, 딥러닝 등 최신 연구에서 -> 과적합 상태에서 복잡도를 계속 높이는것이 나을 수 있음 (어느 한계선을 지나면 검증성능 다시 좋아짐)
