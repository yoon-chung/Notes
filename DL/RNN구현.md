...
* 각 time step에서: 이전 hidden state (hₜ₋₁) + 현재 input (xₜ) → 새 hidden state (hₜ)
* 이전 정보가 hidden state를 통해 다음 단계로 전달됨
```
...
Gate 역할:
• Forget Gate (fₜ): 과거 정보 중 얼마나 "버릴지" 결정 (0~1)
• Input Gate (iₜ):  새 정보를 얼마나 "기억할지" 결정 (0~1)
• Output Gate (oₜ): 얼마나 "출력할지" 결정 (0~1)
```
...
RNN 문제:  x₁ → x₂ → x₃ → ... → x₁₀₀
           ↑
           이 정보가 x₁₀₀ 예측에 거의 영향을 못 미침!

LSTM 해결: Cell State가 별도 경로로 정보를 전달
           ════════════════════════════════════►
           정보 손실 없이 장기 기억 유지!
```
...
| Gate | 역할 | 수식 |
|------|------|------|
| **Forget Gate** | 과거 정보 중 **얼마나 버릴지** | $f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)$ |
| **Input Gate** | 새 정보를 **얼마나 기억할지** | $i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)$ |
| **Output Gate** | 다음 시점으로 **얼마나 전달할지** | $o_t = \sigma(W_o \cdot [h_{t-1}, x_t] + b_o)$ |
...
Gate 역할:
• Reset Gate (rₜ):  이전 정보 중 무엇을 "무시할지" 결정
• Update Gate (zₜ): 새 정보를 얼마나 "반영할지" 결정

※ Cell State 없음 → LSTM보다 단순하고 빠름!
```
...
│               y₁        y₂        y₃        y₄                   │
│                                                                  │
│   * Forward:  왼쪽 → 오른쪽 (과거 정보 반영)                       │
│   * Backward: 오른쪽 → 왼쪽 (미래 정보 반영)                       │
│   * 두 hidden state를 concat하여 사용 → hidden_size × 2           │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
3. I'm ____ hungry, so I can eat more.
   → "not"이 가장 적절! (뒤 문맥 "can eat more"로 유추)

결론: 뒤의 문맥도 예측에 중요한 정보!
```
